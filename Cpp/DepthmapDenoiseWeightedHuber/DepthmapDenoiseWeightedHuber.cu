...
namespace cv { namespace cuda { namespace dtam_denoise {

    static unsigned int arows; // TODO:make sure this is still reentrant

    void loadConstants(
        uint h_rows, uint, uint, uint, float *, float *, float *, float *, float *, float *)
    {
...
#define GENERATE_CUDA_FUNC2D(funcName, arglist, notypes)                                           \
    ...
    {                                                                                              \
        ...
        dim3 dimGrid(                                                                              \
            (acols + dimBlock.x - 1) / dimBlock.x, (arows + dimBlock.y - 1) / dimBlock.y);         \
...
#define GENERATE_CUDA_FUNC2DROWS(funcName, arglist, notypes)                                       \
    ...
    GENERATE_CUDA_FUNC2DROWS(computeG1, (float *pp, float *g1p, float *gxp, float *gyp, int cols),
        (pp, g1p, gxp, gyp, cols))
    {
#if __CUDA_ARCH__ >= 300
        // TODO: make compatible with cuda 2.0 and lower (remove shuffles). Probably through texture
        // fetch
        ...
        //      alpha=3.5;
        //      g1=exp(-alpha*g1);
        //      //hard to explain this without a picture, but breaks are where both neighboring
        //      pixels are near a change gx=max(g1r,g1); gy=max(g1d,g1); gu=gyu;  //upper spring is
        //      the lower spring of the pixel above gd=gy;   //lower spring gr=gx;   //right spring
    ...
    GENERATE_CUDA_FUNC2DROWS(computeG2, (float *pp, float *g1p, float *gxp, float *gyp, int cols),
        (pp, g1p, gxp, gyp, cols))
    {
#if __CUDA_ARCH__ >= 300
        int x = threadIdx.x;
    ...
    // Could be fixed by a second pass for part 2 over the stitch lines, but I don't have time to
    // figure that out right now.
    GENERATE_CUDA_FUNC2DROWS(computeGunsafe,
        (float *pp, float *g1p, float *gxp, float *gyp, int cols), (pp, g1p, gxp, gyp, cols))
    {
#if __CUDA_ARCH__ >= 300
        // TODO: make compatible with cuda 2.0 and lower (remove shuffles). Probably through texture
        // fetch
    ...
    static __global__ void updateQ(float *gqxpt, float *gqypt, float *dpt, float *apt, float *gxpt,
        float *gypt, int cols, float sigma_q, float sigma_d, float epsilon, float theta);
    static __global__ void updateD(float *gqxpt, float *gqypt, float *dpt, float *apt, float *gxpt,
        float *gypt, int cols, float sigma_q, float sigma_d, float epsilon, float theta);

    void updateQDCaller(float *gqxpt, float *gqypt, float *dpt, float *apt, float *gxpt,
        float *gypt, int cols, float sigma_q, float sigma_d, float epsilon, float theta)
    {
        ...
        cudaSafeCall(cudaGetLastError());
        updateQ<<<dimGrid, dimBlock, 0, localStream>>>(
            gqxpt, gqypt, dpt, apt, gxpt, gypt, cols, sigma_q, sigma_d, epsilon, theta);
        cudaSafeCall(cudaGetLastError());
        updateD<<<dimGrid, dimBlock, 0, localStream>>>(
            gqxpt, gqypt, dpt, apt, gxpt, gypt, cols, sigma_q, sigma_d, epsilon, theta);
        cudaSafeCall(cudaGetLastError());
    };
    ...
    //                 float *gxpt, float *gypt, float sigma_q, float sigma_d, float epsilon,
    //                 float theta) {
    //     //TODO: make compatible with cuda 2.0 and lower (remove shuffles). Probably through
    //     texture fetch
    //
    //     //Original pseudocode for this function:
    // //void updateQD(){
    ...
    // //        //load(dh,dn,gxh,gqx);//load here, next(the block to the right), local constant,
    // old x force(with cached multiply)
    // //        dr=dh<<1;
    // //        tmp=dn>>31;
    // //        if (rt)
    // //            dr=tmp;
    // //        qxh=gqx/gxh;
    // //        qxh = (qxh+sigma_q*gxh*(dr-dh))/(1+sigma_q*epsilon);//basic spring force equation
    // f=k(x-x0)
    // //        gqx = saturate(gxh*qxh);//spring saturates (with cached multiply), saturation force
    // proportional to prob. of not an edge.
    // //        gqxpt[pt]=gqx;
    // //
    // //        //qy update
    // //        s[bpt]=dn;
    ...
    //                 dr=tmp;
    //             qx = gqx/gx;
    //             qx = (qx+sigma_q*gx*(dr-dh))/(1+sigma_q*epsilon);//basic spring force equation
    //             f=k(x-x0) gqx = saturate(gx*qx);//spring saturates (with cached multiply),
    //             saturation force proportional to prob. of not an edge.
    //             //gqxpt[pt]=gqx;
    //         }
    //         dpt[pt] = dh;
    //         //qy update
    ...
    GENERATE_CUDA_FUNC2DROWS(updateQ,
        (float *gqxpt, float *gqypt, float *dpt, float *apt, float *gxpt, float *gypt, int cols,
...
//         //qx update
//         float dh,dn,qxh,gx,gqx,qyh,gy,gqy;
//         //load(dh,dn,gxh,gqx);//load here, next(the block to the right), local constant, old x
//         force(with cached multiply) dr=dh<<1; tmp=dn>>31; if (rt)
//             dr=tmp;
//         qxh=gqx/gxh;
//         qxh = (qxh+sigma_q*gxh*(dr-dh))/(1+sigma_q*epsilon);//basic spring force equation
//         f=k(x-x0) gqx = saturate(gxh*qxh);//spring saturates (with cached multiply), saturation
//         force proportional to prob. of not an edge. gqxpt[pt]=gqx;
//
//         //qy update
//         s[bpt]=dn;
//         if(!btm){
...
// }
                ...
                for (; x < cols; x += 32) {
                    ...
                    {
                        float dr;
                        // load(dh,dn,gxh,gqx);//load here, next(the block to the right), local
                        // constant, old x force(with cached multiply)
                        ...
                        qx = (qx + sigma_q * gx * (dr - dh))
                            / (1 + sigma_q * epsilon); // basic spring force equation f=k(x-x0)
                        gqx = gx * saturate(qx);
                        //             gqx = saturate(gx*qx);//spring saturates (with cached
                        //             multiply), saturation force proportional to prob. of not an
                        //             edge.
                        gqxpt[pt] = gqx;
                    }
            ...
            GENERATE_CUDA_FUNC2DROWS(updateD,
                (float *gqxpt, float *gqypt, float *dpt, float *apt, float *gxpt, float *gypt,
                    int cols, float sigma_q, float sigma_d, float epsilon, float theta),
                (gqxpt, gqypt, dpt, apt, gxpt, gypt, cols, sigma_q, sigma_d, epsilon, theta))
            {
#if __CUDA_ARCH__ >= 300
                // TODO: make compatible with cuda 2.0 and lower (remove shuffles). Probably through
                ...
                //         float dh,dn,qxh,gx,gqx,qyh,gy,gqy;
                //         //load(dh,dn,gxh,gqx);//load here, next(the block to the right), local
                //         constant, old x force(with cached multiply) dr=dh<<1; tmp=dn>>31; if (rt)
                //             dr=tmp;
                //         qxh=gqx/gxh;
                //         qxh = (qxh+sigma_q*gxh*(dr-dh))/(1+sigma_q*epsilon);//basic spring force
                //         equation f=k(x-x0) gqx = saturate(gxh*qxh);//spring saturates (with
                //         cached multiply), saturation force proportional to prob. of not an edge.
                //         gqxpt[pt]=gqx;
                //
                //         //qy update
                //         s[bpt]=dn;
                ...
                for (; x < cols; x += 32) {
                    ...
                    __syncthreads(); // can't figure out why this is needed, but it is to avoid
                                     // subtle errors in Qy at the ends of the warp
                }
#endif
            }

}}} // namespace cv::cuda::dtam_denoise
