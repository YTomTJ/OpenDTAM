...
namespace cv { namespace cuda {
    ...
    //!  High level algorithm described by Richard Newcombe, Steven J. Lovegrove, and Andrew J.
    //!  Davison. in "DTAM: Dense tracking and mapping in real-time." Which was in turn based on
    //!  Chambolle & Pock's "A first-order primal-dual algorithm for convex problems with
    ...
    public:
        ...
        virtual GpuMat operator()(InputArray input, float epsilon, float theta) = 0;

        //! In case you want to do these explicitly
        virtual void allocate(
            int rows, int cols, InputArray gx = GpuMat(), InputArray gy = GpuMat())
            = 0;
    ...
    //! The visibleLightImage is a CV_32FC1 grayscale image of the scene, which can be used as a
    //! hint for edge placement.
    CV_EXPORTS Ptr<DepthmapDenoiseWeightedHuber> createDepthmapDenoiseWeightedHuber(
        InputArray visibleLightImage = GpuMat(), Stream cvStream = Stream::Null());
}} // namespace cv::cuda
...
namespace cv { namespace cuda {
    ...
    //! Original algorithm described by Richard Newcombe, Steven J. Lovegrove, and Andrew J.
    ...
    public:
        ...
        DepthmapDenoiseWeightedHuberImpl(
            const GpuMat &visibleLightImage = GpuMat(), Stream cvStream = Stream::Null());
