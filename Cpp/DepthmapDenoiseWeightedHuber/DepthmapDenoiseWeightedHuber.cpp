...
namespace cv { namespace cuda {
    ...
    DepthmapDenoiseWeightedHuberImpl::DepthmapDenoiseWeightedHuberImpl(
        const GpuMat &_visibleLightImage, Stream _cvStream)
    ...
    Ptr<DepthmapDenoiseWeightedHuber> CV_EXPORTS createDepthmapDenoiseWeightedHuber(
        InputArray visibleLightImage, Stream cvStream)
    {
        return Ptr<DepthmapDenoiseWeightedHuber>(
            new DepthmapDenoiseWeightedHuberImpl(visibleLightImage.getGpuMat(), cvStream));
    }

#define FLATALLOC(n, cv)                                                                           \
    ...
    void DepthmapDenoiseWeightedHuberImpl::allocate(
        int _rows, int _cols, InputArray _gxin, InputArray _gyin)
    {
        ...
        if (!(rows % 32 == 0 && cols % 32 == 0 && cols >= 64)) {
            CV_Assert(
                !"For performance reasons, DepthmapDenoiseWeightedHuber currenty only supports "
                 "multiple of 32 image sizes with cols >= 64. Pad the image to achieve this.");
        }
    ...
    void DepthmapDenoiseWeightedHuberImpl::computeSigmas(float epsilon, float theta)
    {
        ...
        */
        ...
        float L = 4; // lower is better(longer steps), but in theory only >=4 is guaranteed to
                     // converge. For the adventurous, set to 2 or 1.44
    ...
    {
        using namespace cv::cuda::dtam_denoise;
    ...
    GpuMat DepthmapDenoiseWeightedHuberImpl::operator()(InputArray _ain, float epsilon, float theta)
    {
        ...
        loadConstants(rows, cols, 0, 0, 0, 0, 0, 0, 0, 0);
        updateQDCaller(gqxpt, gqypt, d, a, gxpt, gypt, cols, sigma_q, sigma_d, epsilon, theta);
