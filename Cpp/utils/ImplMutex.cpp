...
//  The cond var does not guarantee fairness, and may wake threads that were queued AFTER when
//  signal was called instead of the correct one For me these failure modes don't matter
...
int pthread_cond_wait(pthread_cond_t *cv, pthread_mutex_t *external_mutex)
{
    ...
    EnterCriticalSection(&cv->waiters_count_lock_);
    cv->waiters_count_--;
    int last_waiter = result == WAIT_OBJECT_0 + BROADCAST && cv->waiters_count_ == 0;
    LeaveCriticalSection(&cv->waiters_count_lock_);
