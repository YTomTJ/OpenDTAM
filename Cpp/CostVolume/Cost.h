...
public:
    ...
    Cost(const cv::Mat &baseImage, int layers, const cv::Mat &cameraMatrix, const cv::Mat &R,
        const cv::Mat &Tr); // autogenerate default depths
    Cost(const cv::Mat &baseImage, int layers, const cv::Mat &cameraMatrix,
        const cv::Matx44d &cameraPose); // autogenerate default depths
    Cost(const cv::Mat &baseImage, const std::vector<float> &depth, const cv::Mat &cameraMatrix,
        const cv::Mat &R, const cv::Mat &Tr); // use given depths
    Cost(const cv::Mat &baseImage, const std::vector<float> &depth, const cv::Mat &cameraMatrix,
        const cv::Matx44d &cameraPose); // use given depths
...
private:
    cv::Mat_<float>
        dataContainer; // stores the actual data for data*, used for auto allocation behavior
    cv::Mat_<float>
        hitContainer; // stores the actual data for hit*, used for auto allocation behavior
...
private: // NOT PUBLIC!!! JUST NEED TO ACCESS FROM A STATIC CALL. DO NOT USE!
    // A update
