...
// in Cost.cpp
using namespace std;
static inline float fastabs(const float &foo) { return fabs(foo); }
...
void Cost::updateCostL1(const cv::Mat &image, const cv::Matx44d &currentCameraPose)
{
    ...
    for (int n = 0; n < depth.size(); ++n) {
        ...
        cv::Mat_<uchar> mask;
        reproject(cv::Mat_<cv::Vec3f>(image), cameraMatrix, pose, currentCameraPose, depth[n],
            plane, mask); // could be as fast as .00614 using resize instead. Currently runs at
                          // .0156s, or about twice as long
        ...
        // toc();
        // tic();
        for (size_t i = 0, moff = 0, coff = 0, p = 0; i < end;
             p++, moff += 3, i += 3, coff += lstep) { //.0055 - .0060 s
...
void Cost::updateCostL2(const cv::Mat &image, const cv::Matx44d &currentCameraPose)
{
