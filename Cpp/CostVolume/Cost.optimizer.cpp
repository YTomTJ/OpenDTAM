...
void Cost::initOptimization()
{ // must be thread safe in allocations(i.e. don't do any after first time!)
...
void Cost::computeSigmas()
{
    ...
    float L = 4.0; // lower is better(longer steps), but in theory only >=4 is guaranteed to
                   // converge
    lambda = 1.0 / theta;
    alpha = epsilon;
    gamma = lambda;
...
void Cost::cacheGValues()
{
    ...
    cout << "Caching G values"
         << "\n";
    _gbig.create(h + 2, w,
        CV_32FC1); // enough room to safely read off the ends
    ...
    // first get derivatives, but use the kind that we need for our purposes, rather than the built
    // in scharr or sobel derivatives. We will do this by getting the absolute value of the
    // differences to each side of the the current pixel and then taking the max along each
    // direction. The
    Mat gx, gy, g1, g2;
...
/*inline float Cost::aBasic(float* data,float l,float ds,float d){
    int mi=0;
    float mv=1.0/(2.0*theta)*ds*ds*(d-0)*(d-0) + data[0]*lambda; //Literal implementation of Eq.14,
note the datastep^2 factor to scale correctly for(int a=1;a<l;a++){ float
v=1.0/(2.0*theta)*ds*ds*(d-a)*(d-a) + data[a]*lambda; if(v<mv){ mv=v; mi=a;
        }
    }
    if(mi==l-1||mi==0){
        return (float)mi;
...
{
    return 1.0 / (2.0 * theta) * ds * ds * (d - a) * (d - a)
        + data[a]
        * lambda; // Literal implementation of Eq.14, note the datastep^2 factor to scale correctly
    //     return 1.0/(2.0*theta)*(d-a)*(d-a) + data[a]*lambda;//forget the ds^2 factor for better
    //     numerical behavior(sometimes) return std::abs(1.0/(2.0*theta)*ds*ds*(d-a)) +
...
//     }
//     //we're strictly inside the safe region, so no worries about interpolation touching bad
//     points float A=discreteMin[-1]; float B=discreteMin[0]; float C=discreteMin[1]; return
//     (A-C)/(A-2*B+C)*.5+float(discreteMin-loind);
// }
...
static void __attribute__((noinline)) qcore(const float denom, st point, const st pstop, float *kx,
    float *ky, const float *d, const float *gd, const float *gu, const float *gl, const float *gr,
    const float &sigma_q, const unsigned &w);
...
void Cost::optimizeQD()
{
    ...
    for (st i = 1; i < h; i++) {
        ...
        qcore(denom, point, pstop, kx, ky, d, gd, gu, gl, gr, sigma_q, w);
        point = pstop;
    ...
    pstop = 1;
    d[here]
        = (d[here] - sigma_d * (gdown * ky[here] + gright * kx[here] - a[here] / theta)) / denom;
    point++;
    ...
    for (; point < pstop; point++) {
        d[here]
            = (d[here]
                  - sigma_d
                      * (gdown * ky[here] + gleft * kx[left] + gright * kx[here] - a[here] / theta))
            / denom;
    }
    ...
    for (; pstop < corestop;) {
        ...
        d[here] = (d[here]
                      - sigma_d
                          * (gup * ky[up] + gdown * ky[here] + gright * kx[here] - a[here] / theta))
            / denom;
        point++;
        // inner core
        pstop += w - 2;
        for (; point < pstop; point++) {
            d[here] = (d[here]
                          - sigma_d
                              * (gup * ky[up] + gdown * ky[here] + gleft * kx[left]
                                  + gright * kx[here] - a[here] / theta))
                / denom;
        }
        // right core
        pstop++;
        d[here] = (d[here]
                      - sigma_d
                          * (gup * ky[up] + gdown * ky[here] + gleft * kx[left] - a[here] / theta))
            / denom;
        point++;
    }
    assert(point == pstop);
    ...
    for (; point < pstop; point++) {
        d[here] = (d[here]
                      - sigma_d
                          * (gup * ky[up] + gleft * kx[left] + gright * kx[here] - a[here] / theta))
            / denom;
    }
