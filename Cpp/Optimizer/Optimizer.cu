...
#define SEND(type, sym) cudaMemcpyToSymbol(sym, &h_##sym, sizeof(type));
    ...
    void loadConstants(
        uint h_rows, uint h_cols, uint, uint, float *, float *, float *, float *, float *, float *)
    {
...
#define GENERATE_CUDA_FUNC1D(funcName, arglist, notypes)                                           \
    ...
    __device__ static inline float afunc(
        float costval, float theta, float d, float ds, float a, float lambda)
    {
        //    return costval;
        return 1.0f / (2.0f * theta) * ds * ds * (d - a) * (d - a)
            + costval * lambda; // Literal implementation of Eq.14, note the datastep^2 factor to
                                // scale correctly
        //     return 1.0/(2.0*theta)*(d-a)*(d-a) + data[a]*lambda;//forget the ds^2 factor for
        //     better numerical behavior(sometimes) return std::abs(1.0/(2.0*theta)*ds*ds*(d-a)) +
        //     data[a]*lambda;//L1 Version
    }
...
#pragma unroll 4
    ...
    GENERATE_CUDA_FUNC1D(minimizeAshared,
        (float *cdata, float *a, float *d, int rows, int cols, int layers, float theta,
            float lambda),
...
#pragma unroll 32
        for (int z = 0; z < layers; z++) {
            ...
            val = __int_as_float(__float_as_int(val) & 0xFFFFFF00
                | z); // load the depth into the least significant 8 bits of the float
            if (val < minv) {
                minv = val;
                mini = z;
    ...
    static __global__ void computeG1(
        float *pp, float *g1p, float *gxp, float *gyp, float lambda, int cols);
    static __global__ void computeG2(
        float *pp, float *g1p, float *gxp, float *gyp, float lambda, int cols);
    ...
    __global__ void updateQD(float *gqxpt, float *gqypt, float *dpt, float *apt, float *gxpt,
        float *gypt, int cols, float sigma_q, float sigma_d, float epsilon,
        ...
        float *gypt, int cols, float sigma_q, float sigma_d, float epsilon, float theta)
    {
        ...
        //         old x force(with cached multiply) dr=dh<<1; tmp=dn>>31; if (rt)
        //             dr=tmp;
        //         qxh=gqx/gxh;
        //         qxh = (qxh+sigma_q*gxh*(dr-dh))/(1+sigma_q*epsilon);//basic spring force equation
        //         f=k(x-x0) gqx = saturate(gxh*qxh);//spring saturates (with cached multiply),
        //         saturation force proportional to prob. of not an edge. gqxpt[pt]=gqx;
        //
        //         //qy update
        //         s[bpt]=dn;
        //         if(!btm){
        ...
        for (; x < cols; x += 32) {
            ...
            // qx update
            {
                ...
                gqx = saturate(gx * qx); // spring saturates (with cached multiply), saturation
                                         // force proportional to prob. of not an edge.
                // gqxpt[pt]=gqx;
            }
            dpt[pt] = dh;
            // qy update
        ...
        (gqxpt, gqypt, dpt, apt, gxpt, gypt, cols, sigma_q, sigma_d, epsilon, theta))
    {
        ...
        for (; x < cols; x += 32) {
            ...
            __syncthreads(); // can't figure out why this is needed, but it is to avoid subtle
                             // errors in Qy at the ends of the warp
        }
    }

}}} // namespace cv::cuda::dtam_optimizer
