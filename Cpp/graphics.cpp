    ...
    while (!*die) {
        ...
        if (ready) { // deal with imshows
            ...
            if ((autoscale[0] == autoscale[1] && autoscale[0] == 0)) {
                ...
                mat.convertTo(
                    mat, CV_MAKETYPE(mat.type(), mat.channels()), scale, -autoscale[0] * scale);
            }
            mat.convertTo(mat, CV_MAKETYPE(CV_8U, mat.channels()),
                255.0); // use 8 bit so we can have the nice mouse over
            if (mat.rows < 250) {
                name += ":small";
                namedWindow(name, CV_WINDOW_KEEPRATIO | CV_GUI_NORMAL);
            }
            imshow(name, mat);
            waitKey(1); // waitkey must occur here so matrix doesn't fall out of scope because
                        // imshow is dumb that way :(
...
void initGui() { ImplThread::startThread(guiLoop, "Graphics"); }
