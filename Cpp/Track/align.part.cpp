...
void Track::cacheDerivatives()
{
    ...
    //         Mat tmp; hconcat(proj.colRange(Range(0,2)) ,proj.colRange(Range(3,4)) ,
    //         tmp);//convert to 2D since only doing that for ESM proj=tmp;
    //
...
static inline void JacobianCore(Mat &dMdp, Mat &G, Mat &J, Mat &H, int numParams)
{
    // tmp=M*G;
    // J = sumchannels(tmp);
    {
    ...
    H += J * J.t(); // add to the hessian accumulator TODO: this might need to be increased to hold
                    // things as doubles
}

static inline void solveJacobian(Mat &dMdp, Mat &G, Mat &J, Mat &err, Mat &p, int numParams)
{
    int c = dMdp.cols;
    int nb = c / BSZ;
