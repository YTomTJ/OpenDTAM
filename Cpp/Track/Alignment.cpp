...
static bool align_level_largedef_gray_forward(
    const Mat &T, // Total Mem cost ~185 load/stores of image
    const Mat &d, const Mat &_I,
    const Mat &cameraMatrix, // Mat_<double>
    const Mat &_p, // Mat_<double>
    int mode, float threshold, int numParams)
{
    ...
    Mat I(r, c, CV_32FC1);
    {
        ...
        Mat toMerge[3] = { _I, Mat(r, c, CV_32FC1, (float *)gradI.data),
            Mat(r, c, CV_32FC1, ((float *)gradI.data) + r * c) };
        Mat packed;
        merge(toMerge, 3, packed); //(Mem cost: min 3 load, 3 store :6)
        Mat pulledBack;

        remap(packed, pulledBack, baseMap, Mat(), CV_INTER_LINEAR, BORDER_CONSTANT,
            0.0); //(Mem cost:?? 5load, 3 store:8)
        ...
        mixChannels(src, 1, dst, 2, from_to,
            3); // extract the image and the resampled gradient //(Mem cost: min 3 load, 3 store :6)
        ...
        if (cv::countNonZero(I) < rows * cols * FAIL_FRACTION) { // tracking failed!
            ...
            //             bool improved = align_level_largedef_gray_forward(  T,//Total Mem cost
            //             ~185 load/stores of image
            //                                                                 d,
            //                                                                 _I,
    ...
    // TODO: Whole loop cacheable except J multiplies if CV_DTAM_REV (Mem cost whole loop 17/itr:
    // 102)
    for (int paramNum = 0; paramNum < numParams; paramNum++) {
        ...
        Mat outMap(rows, cols, CV_32FC2,
            ((float *)outContainer.data) + rows * cols * 2 * paramNum + OM_OFFSET);
    ...
    Mat dp = (Hinv * (Jsmall * err))
                 .t(); // transpose because we decided that p is row vector (Mem cost 7)
...
//
//             Mat gradPullback;
//             remap( gradimage, gradPullback, baseMap, Mat(), CV_INTER_LINEAR, BORDER_REPLICATE);//
//             Might be faster on 3 channels split(gradPullback,gradLayers); gradient=gradI;
//         }break;
//         case CV_DTAM_REV:
//         {
